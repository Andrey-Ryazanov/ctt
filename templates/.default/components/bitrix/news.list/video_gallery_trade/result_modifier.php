<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
global $APPLICATION;
$dir = $APPLICATION->GetCurDir();
$path=explode('/', $dir);


if (isset($arParams['PARENT_SECTION']) && $arParams['PARENT_SECTION']) {
    $arFilter = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'ID'=>$arParams['PARENT_SECTION'], 'GLOBAL_ACTIVE'=>'Y', '!UF_CONTAINS_VIDEO'=>'');
}
else
{
    $arFilter = Array('IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'GLOBAL_ACTIVE'=>'Y', '!UF_CONTAINS_VIDEO'=>'');
}
$db_list = CIBlockSection::GetList(Array('sort'=>'asc'), $arFilter, true,Array('NAME','ID','UF_MORE_PHOTO','UF_TEXT_URL','UF_NAME', 'UF_CONTAINS_VIDEO'));
while($ar_result = $db_list->GetNext()) {

    if($path[1]=='en' &&  $ar_result["UF_NAME"]){
        $ar_result["NAME"] = $ar_result["UF_NAME"];
    }

    // // Display only one section
    // if (isset($arParams['PARENT_SECTION']))
    // {
    //     if ($ar_result['ID'] === $arParams['PARENT_SECTION'])
    //     {
    //         $arResult['SECTIONS'][]=$ar_result;
    //     }
    // }
    // else
    // {
    //     $arResult['SECTIONS'][]=$ar_result;
    // }
    $arResult['SECTIONS'][]=$ar_result;
}



$items = array();
foreach($arResult['ITEMS'] as $arItem){
    $file1 = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width'=>225, 'height'=>150), BX_RESIZE_IMAGE_EXACT, true);
    $arItem['IMG']['mini'] = $file1;
    $file2 = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width'=>1200, 'height'=>1200), BX_RESIZE_IMAGE_PROPORTIONAL, true);
    $arItem['IMG']['big'] = $file2;
    $items[$arItem['IBLOCK_SECTION_ID']][] = $arItem;
}
$arResult['ITEMS'] =$items;



